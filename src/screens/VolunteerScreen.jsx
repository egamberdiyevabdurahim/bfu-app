    ...
    <Page>
      <div style={{ padding: "calc(var(--safe-t) + 16px) 20px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            <p style={{ color: "var(--text-3)", fontSize: 12, fontFamily: "var(--font-display)", fontWeight: 600, letterSpacing: "0.1em" }}>{t("volunteer.kicker")}</p>
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 800 }}>{t("volunteer.title")}</h1>
          </div>
        ...
        <div style={{ display: "flex", background: "var(--surface-2)", borderRadius: "var(--radius-sm)", padding: 3, marginBottom: 20, gap: 2 }}>
          {[["browse","board.tab.browse"], ["my volunteering","board.tab.myVolunteering"], ["requests","board.tab.requests"]].map(([tab, key]) => (
            ...
            <div key={v.id} className="card" style={{ animation: `fadeUp ${0.1 + i * 0.05}s ease`, cursor: "pointer" }}
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: v.is_member ? 12 : 0 }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 12, color: "var(--text-3)" }}>
                  <Icon name="users" size={12} /> {t("board.volunteersN", { n: v.member_count })}
                </span>
                {v.req_regions?.length > 0 && <span className="tag">{t("board.regionsN", { n: v.req_regions.length })}</span>}
                {v.req_skills?.length > 0 && <span className="tag">{t("board.skillsN", { n: v.req_skills.length })}</span>}
