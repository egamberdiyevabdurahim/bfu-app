  ...
  const validate = () => {
    ...
    if (form.birth_year) {
      ...
      if (isNaN(by) || by < CURRENT_YEAR - 60 || by > CURRENT_YEAR - 10) {
        errs.birth_year = `Must be between ${CURRENT_YEAR - 60} and ${CURRENT_YEAR - 10}`;
      }
    }
  ...
  const handleFetchUsername = async () => {
    ...
    try {
      ...
      if (res.tg_username) {
        ...
        alert("You don't have a Telegram username set.");
      }
    } catch (e) {
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ padding: "52px 24px 16px", flexShrink: 0, borderBottom: "1px solid var(--border)", background: "var(--bg)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 800 }}>Edit Profile</h1>
        </div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {(form.name !== origName || form.surname !== origSurname) && (
            <div style={{ fontSize: 11, color: "#FFB347", background: "rgba(255,179,71,0.1)", borderRadius: 8, padding: "8px 12px" }}>
              ⚠️ Changing your name will update your tag in all your groups
            </div>
          )}
          ...
          {/* Intentions */}
          <div>
            <div className="section-label">Intentions</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <button onClick={() => set("open_to_work", !form.open_to_work)} style={{
                ...
                <div>
                  <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 13 }}>Open to Co-founding / Work</div>
                  <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 2 }}>Join startups or find partners</div>
                </div>
              ...
              <button onClick={() => set("open_to_volunteering", !form.open_to_volunteering)} style={{
                ...
                <span style={{ fontSize: 18 }}>🤝</span>
                <div>
                  <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 13 }}>Open to Volunteering</div>
                  <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 2 }}>Help with community projects</div>
                </div>
          ...
          {/* Telegram Username */}
          <div>
            <div className="section-label">Telegram Username</div>
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ position: "relative", flex: 1 }}>
                ...
                <input className="input-field" value={form.tg_username.replace("@", "")}
                  ...
                  style={{ paddingLeft: 28 }} placeholder="your_username" />
              </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {LANGS.map(l => (
                <button key={l.code} onClick={() => set("language", l.code)} style={{
          ...
          <div>
            ...
            {form.about !== origAbout && (
              <div style={{ fontSize: 11, color: "#4ECDC4", background: "rgba(78,205,196,0.1)", borderRadius: 8, padding: "8px 12px", marginTop: 6 }}>
                ✨ AI will re-analyze your bio after saving
              </div>
            )}
          </div>
      ...
      <div style={{ padding: "12px 24px 36px", flexShrink: 0, borderTop: "1px solid var(--border)", background: "var(--bg)" }}>
        <button className="btn-primary" onClick={handleSave} disabled={loading || updatingTags}>
          {updatingTags ? "Updating group tags..." : loading ? "Saving..." : "💾 Save Changes"}
        </button>
      </div>
    </div>
  );
